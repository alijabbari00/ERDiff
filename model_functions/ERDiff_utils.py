...
def vel_cal(test_trial_vel_tide, VAE_Readout_model, test_latents):
    ...
    with torch.no_grad():
        ...
        r2 = 100 * r2_score(test_trial_vel_tide.reshape((-1, 2)), vel_hat_test.reshape((-1, 2)),
                                                   multioutput='uniform_average')
        rmse = np.sqrt(mean_squared_error(test_trial_vel_tide.reshape((-1, 2)), vel_hat_test.reshape((-1, 2))))
